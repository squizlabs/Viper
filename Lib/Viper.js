...
Viper.prototype = {
    ...
    init: function()
    {
        ...
        this.ViperInputHandler   = new ViperInputHandler(this);
    ...
    _deleteFromRight: function(range)
    {
        ...
        if (range.endOffset === container.data.length) {
            ...
            var eParent = ViperUtil.getFirstBlockParent(cRange.endContainer);
            if (eParent) {
                if (ViperUtil.isChildOf(eParent, this.element) === false) {
                    return;
                }

                var sParent = ViperUtil.getFirstBlockParent(cRange.startContainer);
            ...
            if (sParent) {
                if (ViperUtil.isChildOf(sParent, this.element) === false) {
                    ...
                    if (!firstChild) {
                        ...
                        sParent = ViperUtil.getFirstBlockParent(cRange.startContainer);
                    }
                }
    ...
    _wrapElement: function(parent, tag, callback, attributes)
    {
        ...
        if (parent.nodeType === ViperUtil.TEXT_NODE) {
            ...
            if (ViperUtil.isBlockElement(parent) === false && ViperUtil.hasBlockChildren(parent) === false) {
    ...
    */
    ...
    mouseDown: function(e)
    {
        ...
        if (this.element !== target && ViperUtil.isChildOfElems(target, [this.element]) !== true) {
            inside = false;

            // Ask plugins if its one of their element.
            var pluginName = this.getPluginForElement(target);
            if (!pluginName && ViperUtil.isChildOfElems(target, [this._viperElementHolder]) !== true) {
                this.setEnabled(false);
