...
ViperCopyPastePlugin.prototype = {
    ...
    _removeWordTags: function(content)
    {
        ...
        // Clean up list tags.
        tags = ViperUtil.getTag('ol,ul', tmp);
        var c    = tags.length;
        for (var i = 0; i < c; i++) {
            ViperUtil.removeAttr(tags[i], 'type');
            ViperUtil.removeAttr(tags[i], 'start');
        }
            ...
            if (newList === true) {
                ...
                if (level !== prevLevel) {
                    if (ViperUtil.isset(indentLvl[level]) === true) {
                        ...
                        ul = document.createElement(listType);
                        //ViperUtil.attr(ul, '_viperlistst', 'list-style-type:' + listStyle);
                        li.appendChild(ul);
