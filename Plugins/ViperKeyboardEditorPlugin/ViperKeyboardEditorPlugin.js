...
function ViperKeyboardEditorPlugin(viper)
{
    ...
    this.viper.registerCallback('Viper:editableElementChanged', 'ViperKeywordPlugin', function() {
        ViperUtil.addEvent(self.viper.getViperElement(), 'mousemove', function(e) {
            clearTimeout(t);
            t = setTimeout(function() {
                var line = ViperUtil.getid(self.viper.getId() + '-blockLine');
                ...
                if (!line) {
                    ...
                    line.id = self.viper.getId() + '-blockLine';
                    ViperUtil.setStyle(line, 'position', 'absolute');
                    ViperUtil.setStyle(line, 'height', '0px');
                    ViperUtil.setStyle(line, 'padding', '10px');
                    ViperUtil.setStyle(line, 'z-index', 1000);
                    ViperUtil.setStyle(line, 'cursor', 'pointer');
...
ViperKeyboardEditorPlugin.prototype = {
    ...
    {
        if (elem.id === this.viper.getId() + '-blockLine') {
            return true;
        }
